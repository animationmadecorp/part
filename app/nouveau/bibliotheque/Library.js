...
import { ArrowLeft, BookOpen, Check, Download, FileText, LayoutDashboard, LockKeyhole, MessageSquare, PackageCheck, Play, Search, UserRound, X } from "lucide-react";
...
function LibraryContent({ initialFollowUp = null, initialRequestId = null, initialTab = "Ma bibliothèque", initialEnglishLesson = null, resources = [] }) {
  ...
  return <>
    <div className="am-library-shell">
      <aside className="am-sidebar"><Brand/><p className="am-eyebrow">TON ESPACE, À TON RYTHME</p><nav aria-label="Espace personnel">{tabs.map(([name, Icon]) => <button key={name} aria-current={tab === name ? "page" : undefined} onClick={() => { setTab(name); setOpened(null); }}><Icon size={20}/>{name}</button>)}</nav><div className="am-sidebar-bottom"><p className="am-handnote">Un petit pas compte aussi.</p><Link href="/nouveau"><ArrowLeft size={16}/> Revenir au site</Link><div className="am-user"><span>M</span><div>Mon espace<small>Compte gratuit</small></div></div></div></aside>
...
function ResourceDialog({ resource, completed, modalRef, onClose, onComplete }) {
  ...
  return <div className="am-modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><section ref={modalRef} role="dialog" aria-modal="true" aria-labelledby="am-dialog-title" aria-describedby="am-dialog-description" className={`am-modal am-resource-dialog am-${resource.color}`}><button className="am-modal-close" onClick={onClose} aria-label="Fermer"><X/></button><header className="am-dialog-header"><span className="am-dialog-format">{resource.format}</span><span className="am-eyebrow">{resource.collection}</span><h2 id="am-dialog-title">{resource.title}</h2><p id="am-dialog-description">{resource.description}</p></header><dl className="am-dialog-meta">{resource.metadata?.map(item => <div key={item.label}><dt>{item.label}</dt><dd>{item.value}</dd></div>)}</dl>
      ...
      <div className="am-dialog-actions">{isLesson ? <button className="am-button" disabled={!hasLessonContent || completed} onClick={onComplete}><Check size={17}/>{!hasLessonContent ? "Bientôt disponible" : completed ? "Leçon terminée" : "Marquer comme terminé"}</button> : isVideo ? <button className="am-button" type="button" disabled><Play size={17}/>Bientôt disponible</button> : downloadableFormats.includes(resource.format) ? <button className="am-button" type="button" disabled><Download size={17}/>Téléchargement après remise</button> : null}<button className="am-text-button" onClick={onClose}>Retour à la bibliothèque</button></div>
    </section></div>;
}
