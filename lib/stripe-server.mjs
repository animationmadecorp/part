...
import { businessIdentity } from "../app/nouveau/_data/legal.js";
...
function addInvoiceFields(form, reference) {
  // Stripe owns the invoice number and sends the paid PDF after Checkout.
  // Keep the seller's legal details consistent with the public legal notice.
  form.set("customer_creation", "always");
  form.set("billing_address_collection", "required");
  form.set("name_collection[individual][enabled]", "true");
  form.set("name_collection[business][enabled]", "true");
  form.set("name_collection[business][optional]", "true");
  form.set("tax_id_collection[enabled]", "true");
  form.set("invoice_creation[enabled]", "true");
  form.set("invoice_creation[invoice_data][custom_fields][0][name]", "SIRET");
  form.set("invoice_creation[invoice_data][custom_fields][0][value]", businessIdentity.siret);
  form.set("invoice_creation[invoice_data][footer]", `${businessIdentity.owner} — ${businessIdentity.legalForm} — ${businessIdentity.address}\n${businessIdentity.vat}`);
  form.set("invoice_creation[invoice_data][metadata][orderReference]", reference);
}
...
export async function createStripeCheckoutSession({
  ...
  addInvoiceFields(form, bookingId);
...
export async function createStripeClientCheckoutSession({
  ...
  form.set("payment_method_types[0]", "card");
  addInvoiceFields(form, requestId);
  form.set("success_url", successUrl);
  form.set("cancel_url", cancelUrl);
...
export async function getStripeInvoice(invoiceId, env = process.env) {
  const { secretKey } = requireStripeCapability(env, "checkoutEnabled", "Stripe invoice configuration is closed");
  if (!/^in_[A-Za-z0-9]+$/.test(invoiceId || "")) throw safeError("Invalid Stripe invoice", 400);
  return stripeGet(`/invoices/${encodeURIComponent(invoiceId)}`, secretKey);
}

export async function getStripePaymentReceipt(paymentIntentId, env = process.env) {
  const { secretKey } = requireStripeCapability(env, "checkoutEnabled", "Stripe receipt configuration is closed");
  if (!/^pi_[A-Za-z0-9]+$/.test(paymentIntentId || "")) throw safeError("Invalid Stripe payment intent", 400);
  const paymentIntent = await stripeGet(`/payment_intents/${encodeURIComponent(paymentIntentId)}`, secretKey);
  if (paymentIntent.id !== paymentIntentId || paymentIntent.status !== "succeeded") return null;
  ...
  const charge = await stripeGet(`/charges/${encodeURIComponent(chargeId)}`, secretKey);
  if (charge.id !== chargeId || charge.payment_intent !== paymentIntentId || charge.paid !== true) return null;
  return charge.receipt_url || null;
}

export async function getStripeCreditNotes(invoiceId, env = process.env) {
  const { secretKey } = requireStripeCapability(env, "checkoutEnabled", "Stripe invoice configuration is closed");
  if (!/^in_[A-Za-z0-9]+$/.test(invoiceId || "")) throw safeError("Invalid Stripe invoice", 400);
  return stripeGet(`/credit_notes?invoice=${encodeURIComponent(invoiceId)}&limit=100`, secretKey);
}

// A Stripe refund does not cancel a finalized invoice. Link each successful
// refund to one credit note, using the refund ID as Stripe's idempotency key.
export async function syncCreditNotesForRefundedCharge({ chargeId, paymentIntentId, env = process.env }) {
  const { secretKey } = requireStripeCapability(env, "webhookEnabled", "Stripe refund configuration is closed");
  if (!/^ch_[A-Za-z0-9]+$/.test(chargeId || "") || !/^pi_[A-Za-z0-9]+$/.test(paymentIntentId || "")) {
    throw safeError("Invalid Stripe refund identifiers", 400);
  }
  const sessions = await stripeGet(`/checkout/sessions?payment_intent=${encodeURIComponent(paymentIntentId)}&limit=2`, secretKey);
  const checkout = sessions?.data?.find((item) => item.payment_intent === paymentIntentId && item.metadata?.userId);
  if (!checkout || checkout.invoice_creation?.enabled !== true) return { skipped: true };
  if (!checkout.invoice) throw safeError("Stripe invoice is still being created", 503, { retryable: true });
  const invoiceId = typeof checkout.invoice === "string" ? checkout.invoice : checkout.invoice.id;
  if (!/^in_[A-Za-z0-9]+$/.test(invoiceId || "")) throw safeError("Invalid Stripe invoice", 503, { retryable: true });
  const [refunds, existingNotes] = await Promise.all([
    stripeGet(`/refunds?charge=${encodeURIComponent(chargeId)}&limit=100`, secretKey),
    stripeGet(`/credit_notes?invoice=${encodeURIComponent(invoiceId)}&limit=100`, secretKey),
  ]);
  if (refunds.has_more || existingNotes.has_more) throw safeError("Stripe refund list needs pagination", 503, { retryable: true });
  const linkedRefunds = new Set((existingNotes.data || []).filter((note) => note.status === "issued")
    .flatMap((note) => note.refunds || []).map((item) => typeof item.refund === "string" ? item.refund : item.refund?.id));
  let created = 0;
  for (const refund of refunds.data || []) {
    if (refund.status !== "succeeded" || linkedRefunds.has(refund.id)) continue;
    if (!/^re_[A-Za-z0-9]+$/.test(refund.id || "") || !Number.isSafeInteger(refund.amount) || refund.amount <= 0) {
      throw safeError("Invalid Stripe refund", 503, { retryable: true });
    }
    const form = new URLSearchParams();
    form.set("invoice", invoiceId);
    form.set("amount", String(refund.amount));
    form.set("refunds[0][refund]", refund.id);
    form.set("refunds[0][amount_refunded]", String(refund.amount));
    await stripeRequest("/credit_notes", secretKey, form, { idempotencyKey: `credit-note:${refund.id}` });
    created += 1;
  }
  return { created };
}
