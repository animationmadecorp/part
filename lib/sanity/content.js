...
import { normalizeResourceGallery, normalizeResourcePortableText } from "./resource-core.mjs";
...
function normalizeResource(value) {
  ...
  const coverUrl = safeArticleUrl(value.coverUrl, { image: true });
  return {
    ...presentation,
    downloadUrl: accessRule === "free" && /^https:\/\/cdn\.sanity\.io\/files\/[a-z0-9]+\/[a-z0-9_-]+\/[^/?#]+\.(?:zip|pdf)$/.test(value.downloadUrl || "") ? value.downloadUrl : null,
    presentationSlug: /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(value.presentationSlug || "") ? value.presentationSlug : null,
    ...
    coverUrl,
    coverAlt: stringOr(value.coverAlt),
    coverAssetRef: stringOr(value.coverAssetRef),
    gallery: normalizeResourceGallery(value.gallery, title ? `Aperçu de ${title}` : undefined),
    recap: normalizeResourcePortableText(value.recap),
    installation: normalizeResourcePortableText(value.installation),
    body: normalizeResourcePortableText(value.body),
    technicalDetails: Array.isArray(value.technicalDetails)
      ? value.technicalDetails
        .map((item) => ({ label: stringOr(item?.label), value: stringOr(item?.value) }))
        .filter((item) => item.label || item.value)
      : [],
    license: stringOr(value.license),
